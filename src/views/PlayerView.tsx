...
export default function PlayerView() {
  ...
  const onError = useCallback((e: YouTubeEvent<number>) => {
  ...
  const seekWithPreRoll = useCallback((targetTime: number) => {
    const player = playerRef.current
    if (!player) return
    ...
    try {
      ...
      preRollFadeRef.current = setInterval(() => {
        ...
        try { player.setVolume(Math.min(vol, originalVolume)) } catch { /* */ }
        if (step >= steps) {
          if (preRollFadeRef.current) clearInterval(preRollFadeRef.current)
  ...
  const handleForward = useCallback(() => {
    ...
    const next = bookmarks.find((b) => b.time > time && b.time <= time + 10)
    if (next) {
  ...
  }, [currentTime, duration, getBookmarks, seekWithPreRoll])

  const handleBackward = useCallback(() => {
    const bookmarks = getBookmarks()
    const time = currentTime
    // Find previous bookmark within 10s
    const prev = [...bookmarks].reverse().find((b) => b.time < time && b.time >= time - 10)
    if (prev) {
  ...
  }, [currentTime, getBookmarks, seekWithPreRoll])
  ...
  const handleBookmarkTap = useCallback((time: number) => {
  ...
  const renderButton = (id: ButtonId) => {
    switch (id) {
      ...
      case 'forward':
        return <TileButton key={id} label="Forward" icon="⏩" onClick={handleForward} />
      case 'backward':
        return <TileButton key={id} label="Backward" icon="⏪" onClick={handleBackward} />
      case 'addBookmark':
        return <TileButton key={id} label="Add Bookmark" icon="🔖" onClick={() => addBookmark(currentTime)} />
      case 'removeBookmark':
        return <TileButton key={id} label="Remove Bookmark" icon="✖" onClick={() => removeBookmarkNearTime(currentTime)} />
      case 'speedUp':
        return <TileButton key={id} label={`Speed Up (${playbackRate}×)`} icon={`${playbackRate}× ▲`} onClick={speedUp} />
      case 'slowDown':
        return <TileButton key={id} label={`Slow Down (${playbackRate}×)`} icon={`${playbackRate}× ▼`} onClick={slowDown} />
      case 'autoLoop':
        return <TileButton key={id} label={looping ? 'Clear Loop' : 'Auto Loop'} icon={looping ? '⏹' : '🔁'} active={looping} onClick={handleAutoLoop} />
      case 'manualLoop':
        return (
          <TileButton
            key={id}
            label={looping ? 'Clear Loop' : manualLoopPicking ? 'Picking…' : 'Manual Loop'}
      ...
      case 'settings':
        return <TileButton key={id} label="Settings" icon="⚙" onClick={() => navigate(`/settings/${videoId}`)} />
      case 'submenu':
        return <TileButton key={id} label="Menu" icon={submenuOpen ? '✕' : '☰'} active={submenuOpen} onClick={toggleSubmenu} />
      case 'favourite':
        return <TileButton key={id} label={isFavourite ? 'Unfavourite' : 'Favourite'} icon={isFavourite ? '★' : '☆'} active={isFavourite} onClick={() => videoId && toggleFavourite(videoId)} />
      case 'openYouTube':
        return (
          <TileButton
            key={id}
  ...
  return (
    <div className="player">
      ...
      <div className="player__status">
        ...
        {manualLoopPicking && (
          <span className="player__loop-label">Tap timeline to set loop points…</span>
        )}
      </div>

      <div className="player__grid" style={{ '--tile-size': `${tileSize}px` } as React.CSSProperties}>
        {visibleButtons.map(renderButton)}
      </div>

      <div className="player__back">
        <button className="player__back-btn" onClick={() => navigate('/front')}>← Back</button>
        <a
